...
void Marker::update(const Booster& booster, const Platform& platform, const sf::RenderWindow& window) {
    ...
    // Фиксированные отступы в пикселях с учетом масштаба
    const float sideMargin = 50.f * (viewArea.width / 1920.f);   // Расстояние от боковых краёв
    const float bottomMargin = 40.f * (viewArea.height / 1080.f); // Расстояние от нижнего края
